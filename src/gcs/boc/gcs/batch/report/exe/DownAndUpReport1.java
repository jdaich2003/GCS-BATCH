...
/**
 * 从ftp服务器下载、上传各系统报表(第一批)
 * 
 * @author daic
 * 
 */
public class DownAndUpReport1 {

	/**
	 * @param args
	 */
	private static Logger logger = Logger.getLogger(DownAndUpReport1.class);
...
//			//  录入明细报表*************************************
			ReportTemplateLoader loader = ReportTemplateParser.getInstance()
					.getConfig("INPUT");
			CSVReprotRender render = new CSVReprotRender(loader, conditions);
			render.process();
			// 录入拒绝明细细报表*************************************
			loader = ReportTemplateParser.getInstance()
					.getConfig("INPUT_REJECT");
			render = new CSVReprotRender(loader, conditions);
			render.process();
			// 已上传发卡明细报表*************************************
			loader = ReportTemplateParser.getInstance()
					.getConfig("UPLOAD");
			...
			// 分行柜台交易明细报表*************************************
			render = new CSVReprotRender(ReportTemplateParser
...
//			proc.process();
			...
			// 上传前一天日期，VISAEU第二批
			paramsUp.setDate(rbd.getBatchDate(-1));
			ftpPut.ftpPutForReport(paramsUp);
